...
const Navber = (props) => {
  ...
  const navberRef = useRef(null); // Ref for the menu container
  ...
  const navigateAndCloseNavber = (path) => {
    closeNavber(); // Close the hamburger menu
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Check if the click is outside the navber and not on the toggle button
      if (
        navberRef.current &&
        ...
        >
          <div className="col-span-1 flex flex-col text-gray-600">
            <span
              ...
              onClick={(e) => navigateAndCloseNavber("/")}
            >
              Shop
            </span>
            <span
              className="font-medium text-lg tracking-widest hover:text-gray-800 hover:bg-gray-200 px-3 py-2 rounded-lg cursor-pointer"
              onClick={(e) => navigateAndCloseNavber("/blogs")}
            >
              Blog
            </span>
            {!localStorage.getItem("userEmail") &&
            ...
            <span
              className="font-medium text-lg tracking-widest hover:text-gray-800 hover:bg-gray-200 px-3 py-2 rounded-lg cursor-pointer"
              onClick={(e) => navigateAndCloseNavber("/contact-us")}
            >
              Contact us
            </span>
          </div>
